...
type caractMap=RECORD
    nbCasesNavires:integer;
    car:integer;
    end;
...
var porteAvion, croiseur, contreCroiseur, sousMarin, torpilleur:bateau;
...
BEGIN
 //initi variables
    ...
    contreCroiseur.taille:=3;
    contreCroiseur.nom:='contre croiseur';
    sousMarin.taille:=contreCroiseur.taille;
    torpilleur.taille:=2;
    ...
    joueur1.nb:=1;
    joueur1.nom:=' ';
    joueur1.map:=mapA_vide(joueur1.map, carVide);
    joueur1.suiviMap:=mapA_vide(joueur1.suiviMap, carVide);
    joueur1.victoire:=false;
    joueur2.nb:=2;
    joueur2.nom:=' ';
    joueur2.map:=mapA_vide(joueur2.map, carVide);
    joueur2.suiviMap:=mapA_vide(joueur2.suiviMap,carVide);
    ...
    readln;
    clrscr;
    joueur1.nom:=recupNom(joueur1.nb);
    joueur2.nom:=recupNom(joueur2.nb);




   //init mapJoueur1
    affichMap(joueur1.map);
    gotoxy(45,3);
    writeln('Voici votre map, ',joueur1.nom);
    gotoxy(26,4);
    ...
    joueur1.map:=initMap(joueur1.map, carVide, sousMarin.nom, sousMarin.taille);
    joueur1.map:=initMap(joueur1.map, carVide, croiseur.nom, croiseur.taille);
    joueur1.map:=initMap(joueur1.map, carVide, porteAvion.nom, porteAvion.taille);
    joueur1.map:=initMap(joueur1.map, carVide, contreCroiseur.nom, contreCroiseur.taille);
    joueur1.map:=initMap(joueur1.map, carVide, torpilleur.nom, torpilleur.taille);
    readln;
    clrscr;

    //init mapJoueur2
    affichMap(joueur2.map);
    gotoxy(45,3);
    writeln('Voici votre map, ',joueur2.nom);
    gotoxy(26,4);
    writeln('Pour commencer le placement des differents navires, veuillez appuyer sur entrer');
    readln;
    clrscr;
        //appel des fcts qui permettent la mise en place de chaque navire sur la map
   joueur2.map:=initMap(joueur2.map, carVide, porteAvion.nom, porteAvion.taille);
   joueur2.map:=initMap(joueur2.map, carVide, croiseur.nom, croiseur.taille);
   joueur2.map:=initMap(joueur2.map, carVide, contreCroiseur.nom, contreCroiseur.taille);
   joueur2.map:=initMap(joueur2.map, carVide, sousMarin.nom, sousMarin.taille);
   joueur2.map:=initMap(joueur2.map, carVide, torpilleur.nom, torpilleur.taille);
    readln;
    clrscr;
...
//session de tirs se repetant jusqu'a un vainqueur soit désigné. -->Condition : plus aucun bateau chez l'adversaire
    ...
    repeat
        ...
        gotoxy(34,5);
        writeln('Ou voulez-vous attaquer, tres cher(e) ',joueur1.nom,' ?');
        joueur2.map:=tirMap(joueur2.map); //fct qui appelle d'autres fcts permettant de tirer sur la map du joueur 2
        joueur2.suiviMap:=suiviTirMap(joueur2.map, joueur2.suiviMap);  //map de suvi de tirs(reussis) mise a jour et affichage de cette derniere
        affichMap(joueur2.suiviMap);
        joueur1.victoire:=estVictoire(joueur2.map, map.nbCasesNavires);  //booleen permettant de savoir si le joueur 1 a gagné la partie ou non
        readln;
        clrscr;
        ...
        readln;
        clrscr;
    until((joueur1.victoire=true) or (joueur1.victoire=true));
...
//fin de la partie
    ...
    if joueur1.victoire=true then
        begin
        gotoxy(33,13);
        writeln('Félicitation ',joueur1.nom,', vous gagnez la partie');
        end
    else if joueur1.victoire=true then
        begin
        gotoxy(33,13);
